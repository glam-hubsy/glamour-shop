...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      if (filterMode === "category" && category !== "all") {
        query = query.eq("category", category);
      }
      if (filterMode === "usage" && usage !== "all") {
        query = query.eq("usage_area", usage);
      }
      if (filterMode === "brand" && brand !== "all") {
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
              filterMode === tab.key
                ...
                : "bg-white text-gray-500 border hover:bg-rose-50"
            }`}
          >
            {isAr ? tab.ar : tab.en}
          </button>
